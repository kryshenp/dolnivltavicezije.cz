...
var Grid = (function() {
    ...
    $items.on('click', 'span.map-og-close', function() {
      hidePreview();
      $(this).trigger('map-og-deselect');
      return false;
      ...
      if (current === $li.index()) {
        hidePreview()
        $li.trigger('map-og-deselect');
      } else {
        var previousSelection = current;
        showPreview($li);
        if (previousSelection == -1) {
          $grid.trigger('map-og-openpreview');
        }
        $li.trigger('map-og-select');
      }
      return false;
    });
  }
	...
	Preview.prototype = {
		create : function() {
			...
			this.$href = $( '<a href="#"></a>' );
			this.$details = $( '<div class="map-og-details"></div>' ).append( this.$title, this.$description );
			this.$loading = $( '<div class="map-og-loading"></div>' );
      this.$fullimage = $( '<div class="map-og-fullimg"></div>' ).append( this.$loading ).append($('<div class="map-og-details-left" style="display:none">'));
      this.$closePreview = $( '<span class="map-og-close"></span>' );
      this.$previewInner = $( '<div class="map-og-expander-inner"></div>' ).append( this.$closePreview, this.$fullimage, this.$details );
      this.$previewLeft = $('<div class="map-og-previous"></div>');
      this.$previewRight = $('<div class="map-og-next"></div>');
      this.$previewEl = $( '<div class="map-og-expander"></div>' ).append( this.$previewInner, this.$previewLeft, this.$previewRight );
		...
		update : function( $item ) {
			...
			if( current !== -1 ) {
				...
				$currentItem.removeClass( 'map-og-expanded' );
				this.$item.addClass( 'map-og-expanded' );
      ...
      $('.map-og-previous, .map-og-next').on('click', function() {
        if ($(this).is('.map-og-previous')) {
          goLeft();
        } else {
          goRight();
        }
      ...
      $(document).on('keyup.og', function(e) {
		...
		close : function() {
      $('.map-og-previous, .map-og-next').off('click');
      $(document).off('keyup.og');

      var self = this,
        onEndFn = function() {
          self.$item.removeClass( 'map-og-expanded' );
		...
		setHeights : function() {
			...
			var self = this,
				onEndFn = function() {
					...
					self.$item.addClass( 'map-og-expanded' );
				};
...
function reflow($container) {
  var options = $container.data('map-og-options');
  var $ul = $container.find('ul.map-og-grid');
...
var createExpandableGrid = function(options, images) {
  ...
  $(this).data('map-og-options', options);

  var $ul = $('<ul class=map-og-grid>').append($(lis).hide());
  ...
  this.on('map-og-deselect', function() {
...
var deselect = function(_) {
  $(this).data('map-og-grid').hidePreview();
};
...
var selectImage = function(_, id) {
  ...
  $(this).data('map-og-grid').showPreview($li);
  return true;
};

var selectedId = function() {
  return $(this).find('li.map-og-expanded').data('image-id');
};

$(window).on('resize', function( event ) {
  $('ul.map-og-grid').each(function(_, ul) {
    reflow($(ul).parent());
  });
});
